...
def fenleiqi(w1_1,b1_1,INPUT,lr3,iterations3,real_one_hot_label):
    loss_data=[]
    loss=0
    for i in range(0,iterations3):
        z1_1=np.dot(w1_1,INPUT)+b1_1#12*30000
        a1_1=softmax(z1_1)#12*30000
        M = np.max(z1_1, axis=0)
        loss = np.sum(-real_one_hot_label * ((z1_1 - M) - np.log(np.sum(np.exp(z1_1 - M), axis=0)))) / m
        loss_data.append(loss)
        if loss<0.1e-10:
            break
        else:
            dz1_1=a1_1-real_one_hot_label
            dw1=np.dot(dz1_1,INPUT.T)#12*9
            db1=np.sum(dz1_1,axis=1,keepdims=True)/m#12*1
            w1_1=w1_1-lr3*dw1
            b1_1=b1_1-lr3*db1
    return w1_1,b1_1,loss,loss_data


#全局微调
def global_fine_tuning(w1_1,w2_2,w3_3,w4_4,w5_5,b1_1,b2_2,b3_3,b4_4,b5_5,INPUT,lr2,iterations2,l):
    loss_data = []
    train_accuracy_rate = []
    loss=0
    for i in range(0,iterations2):
        ...
        loss = np.sum(-train_one_hot_label * ((z5_5 - M) - np.log(np.sum(np.exp(z5_5 - M), axis=0)))) / m + 0.5*l*(np.sum(np.square(w1_1))+np.sum(np.square(w2_2))+np.sum(np.square(w3_3))+np.sum(np.square(w4_4))+np.sum(np.square(w5_5)))/m
        loss_data.append(loss)
...
#分类器层预训练
w9,b9,loss_fenleiqi,loss_data_fenleiqi=fenleiqi(w1_1=w9,b1_1=b9,INPUT=AE4_code,lr3=0.03,iterations3=3000,real_one_hot_label=train_one_hot_label)
...
#全局微调
w1,b1,w3,b3,w5,b5,w7,b7,w9,b9,loss_fine_tuning,loss_data_fine_tuning,train_accuracy_rate=global_fine_tuning(w1_1=w1,w2_2=w3,w3_3=w5,w4_4=w7,w5_5=w9,b1_1=b1,b2_2=b3,b3_3=b5,b4_4=b7,b5_5=b9,INPUT=trainT,lr2=0.001,iterations2=5000,l=1)

...
train=np.vstack((normal880_3[:ending_number1],duanmo880_3[:ending_number1],dianshi880_3[:ending_number1],dianmo880_3[:ending_number1],duanchi880_3[:ending_number1],mosun880_3[:ending_number1]))#2400*9
test=np.vstack((normal880_3[ending_number1:ending_number2],duanmo880_3[ending_number1:ending_number2],dianshi880_3[ending_number1:ending_number2],dianmo880_3[ending_number1:ending_number2],duanchi880_3[ending_number1:ending_number2],mosun880_3[ending_number1:ending_number2]))#1200*9


#数据归一化
scaler=MinMaxScaler()
trainT=scaler.fit_transform(train).T#最小最大归一化MinMaxScaler函数是对列进行处理的，这里对齿轮箱的每种性状进行归一化处理
...
def sigmoid(z):                             #定义sigmoid函数
    ...
    indices_pos=np.nonzero(z>=0)#得到z中大于等于0的元素的行标列标
    indices_neg=np.nonzero(z<0)#得到z中小于0的元素的行标列标
    hu=np.zeros((n_rows,n_cols))#创建一个和z形状相同的全0矩阵
    hu[indices_pos]=1/(1+np.exp(-z[indices_pos]))#z中大于等于0的元素单独运算，并将值存入相应的位置
    hu[indices_neg]=np.exp(z[indices_neg])/(1+np.exp(z[indices_neg]))#z中小于0的元素单独运算，并将值存入相应的位置
    return hu
...
def softmax(x):
    k=x.shape[1]#返回x的列数
    M=np.max(x, axis=0)#返回每一列的最大值，M是一个列表
    c=x-M#利用矩阵运算的广播机制，这里M被广播成与x形状相同的矩阵，且M中每一列的元素相同
    ex=np.exp(c.T)#将c转置后做指数运算
    ex_sum=np.sum(ex,axis=1)#返回每一行的和，ex_sum是一个列表
    count=[]
    for i in range(0,k):
        count.append(ex[i]/ex_sum[i])#ex里的每一行的每个元素除以ex_sum里对应的值，并将其存储在count里，count形状和ex相同，count是一个存储了多个列表的列表
    count=np.array(count).T#将count变为矩阵再将其转置
    return count
...
def accuracy_count(predict_value,real_label):
    ...
    output = np.argmax(predict_value, axis=0)#返回predict_value每一列最大值的索引
    count = 0
...
#初始化权重和偏置，随机权重符合标准正态分布
np.random.seed(1)
w1=np.random.randn(ec1,x)#12*9
b1=np.zeros((ec1,1))#12*1
np.random.seed(1)
w2=np.random.randn(x,ec1)#9*12
b2=np.zeros((x,1))#12*1
np.random.seed(1)
w3=np.random.randn(ec2,ec1)#10*12
b3=np.zeros((ec2,1))#10*1
np.random.seed(1)
w4=np.random.randn(ec1,ec2)#12*10
b4=np.zeros((ec1,1))#12*1
np.random.seed(1)
w5=np.random.randn(ec3,ec2)#8*10
b5=np.zeros((ec3,1))#8*1
np.random.seed(1)
w6=np.random.randn(ec2,ec3)#10*8
b6=np.zeros((ec2,1))#10*1
np.random.seed(1)
w7=np.random.randn(ec4,ec3)#4*8
b7=np.zeros((ec4,1))#4*1
w8=np.random.randn(ec3,ec4)#8*4
b8=np.zeros((ec3,1))#8*1
w9=np.random.randn(y,ec4)#6*4
b9=np.zeros((y,1))#6*1
...
def AE(w1_1,w2_2,b1_1,b2_2,INPUT,lr1,iterations1):
    ...
    for i in range(0,iterations1):
        ...
        if loss<0.1e-10:
            ...
            dz2_2=(a2_2-INPUT)*dsigmoid(a2_2)
            dw2=np.dot(dz2_2,a1_1.T)
            db2=np.sum(dz2_2,axis=1,keepdims=True)/m
            dz1_1=np.dot(w2_2.T,dz2_2)*dsigmoid(a1_1)
            dw1=np.dot(dz1_1,INPUT.T)
            db1=np.sum(dz1_1,axis=1,keepdims=True)/m
            w1_1=w1_1-lr1*dw1
            b1_1=b1_1-lr1*db1
            w2_2=w2_2-lr1*dw2
            b2_2=b2_2-lr1*db2
